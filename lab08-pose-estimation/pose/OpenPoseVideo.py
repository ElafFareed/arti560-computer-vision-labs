...
parser = argparse.ArgumentParser(description="Run OpenPose keypoint detection on a video")
parser.add_argument("--device", default="cpu", choices=["cpu", "gpu"], help="Device to inference on")
parser.add_argument("--video_file", default="sample_video.mp4", help="Input video path")
...
if not cap.isOpened():
    raise FileNotFoundError(f"Could not open video file: {input_source}")
...
output_file = f"{save_name}_openpose.avi"
